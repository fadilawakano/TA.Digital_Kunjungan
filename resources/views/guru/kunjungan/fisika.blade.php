...
<div class="container mx-auto px-4 py-6">
    <h3 class="text-xl font-bold mb-4">Data Kunjungan Laboratorium Fisika</h3>
    ...
    <table class="min-w-full bg-white shadow-md rounded border">
        ...
        <tbody>
            @foreach ($kunjunganFisika as $kunjungan)
                <tr>
                    ...
                    <td class="p-2 border">{{ $kunjungan->alat }}</td>
                    <td class="p-2 border">{{ $kunjungan->jumlah_alat }}</td>
                    <td class="p-2 border">
                    ...
                    <td class="p-2 border">
                        @if (!$kunjungan->status_verifikasi)
                            <form method="POST" action="{{ route('fisika.verifikasi', $kunjungan->id) }}">
                                @csrf
