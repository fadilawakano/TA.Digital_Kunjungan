...
<div class="p-6"
    ...
    <div class="container mx-auto px-4 py-6">
        <h1 class="text-2xl font-bold mb-6">Data Kunjungan Murid ke Lab Fisika</h1>

        {{-- Filter & Tombol Cetak --}}
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4">
            <form action="{{ route('fisika.filter', 'murid') }}" method="GET" class="flex flex-wrap items-center gap-2">
            ...
            <template x-if="tab === 'kunjungan'">
                <a 
                    :href="`{{ route('fisika.cetak.fisika.murid') }}?filter_jenis=${filterJenis}` +
                ...
                <tbody>
                    @forelse ($data as $item)
                        <tr class="border-t hover:bg-[#f7e8d3]">
                            ...
                            <td class="px-4 py-2">{{ $item->judul_materi }}</td>
                            <td class="px-4 py-2">
