...
<div class="p-6"
    ...
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4">
            <form action="{{ route('kimia.filter', 'murid') }}" method="GET" class="flex flex-wrap items-center gap-2">
            ...
            <template x-if="tab === 'kunjungan'">
                <a 
                    :href="`{{ route('kimia.cetak.kimia.murid') }}?filter_jenis=${filterJenis}` +
        ...
        <div x-show="tab === 'kunjungan'" class="bg-[#FAF1E6] rounded shadow p-4 overflow-x-auto">
            <table class="table-auto w-full mt-4">
                ...
                <tbody>
                    @forelse ($kunjunganKimia as $data)
                        <tr class="border-t hover:bg-[#f7e8d3]">
                            ...
                            <td class="px-4 py-2">
                                ...
                                @else
                                    <div class="flex gap-2 flex-wrap">
                                        <form action="{{ route('kimia.verifikasi', $data->id) }}" method="POST">
                                            @csrf
                            ...
                            <td class="px-4 py-2">
                                <form action="{{ route('kimia.kunjungan.destroy', $data->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                    @csrf
                                    @method('DELETE')
